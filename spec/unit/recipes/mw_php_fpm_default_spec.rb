...
describe 'mw_php_fpm_test::default' do
  before do
  ...
  context 'compiling the test recipe' do
    it 'converges successfully' do
      expect(chef_run).to create_mw_php_fpm('sample').with(
  ...
  context 'stepping into mw_php_fpm[sample] resource' do
    ...
    it 'creates a php-fpm pool' do
      expect(chef_run).to create_template("#{chef_run.node['php-fpm']['pool_conf_dir']}/sample.conf")
    end
  end
end
